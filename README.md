...
- **锁价采购单**：只能把资质已通过、有货且数量达到起订量的报价加入采购单；首次加入时锁定单价，重复加入同一报价只累加数量；采购单并列展示锁价、当前价、差额与不可采购原因。
...
| GET | `/api/v1/purchase-order` | 当前用户采购单（锁价、当前价、差额、可采购状态） |
| POST | `/api/v1/purchase-order/items` | 报价加入采购单，body：`{"offer_id":1,"quantity":10}`；重复加入累加数量 |
| PATCH | `/api/v1/purchase-order/items/:id` | 调整采购数量，body：`{"quantity":20}`；不可采购条目会被拒绝 |
