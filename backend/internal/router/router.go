...
func New(db *gorm.DB, logger *slog.Logger, jwtSecret string) *gin.Engine {
	...
	purchaseOrder := handler.NewPurchaseOrderHandler(service.NewPurchaseOrderService(repository.NewPurchaseOrderRepository(db), repository.NewOfferRepository(db)), v)
	...
	api.GET("/purchase-order", purchaseOrder.List)
	api.POST("/purchase-order/items", purchaseOrder.AddItem)
	api.PATCH("/purchase-order/items/:id", purchaseOrder.UpdateItem)
