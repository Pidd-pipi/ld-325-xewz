...
export const api = {
  ...
  purchaseOrder: () => request<PurchaseOrder>('/purchase-order'),
  addPurchaseOffer: (offerId: number, quantity: number) => request('/purchase-order/offers', { method: 'POST', body: JSON.stringify({ offer_id: offerId, quantity }) }),
  updatePurchaseQuantity: (itemId: number, quantity: number) => request(`/purchase-order/items/${itemId}/quantity`, { method: 'PATCH', body: JSON.stringify({ quantity }) }),
};
