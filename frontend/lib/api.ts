...
export const api = {
  ...
  getPurchaseOrder: () => request<PurchaseOrder>('/purchase-order'),
  addPurchaseItem: (offerId: number, quantity: number) => request<PurchaseOrder>('/purchase-order/items', { method: 'POST', body: JSON.stringify({ offer_id: offerId, quantity }) }),
  updatePurchaseItem: (itemId: number, quantity: number) => request<PurchaseOrder>(`/purchase-order/items/${itemId}`, { method: 'PATCH', body: JSON.stringify({ quantity }) }),
};
